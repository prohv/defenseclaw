...
Antigravity v1 publishes only the hooks surface in its documented configuration. MCP servers, skills, rules, plugins, and agent assets are not exposed as documented local-install surfaces yet, so DefenseClaw treats those as unsupported for Antigravity and hides the corresponding panels while `claw.mode=antigravity`. If Google publishes additional install contracts, DefenseClaw will surface them through a contract bump.

`claw.workspace_dir` is intentionally not honored by Antigravity setup. Antigravity merges discovered `hooks.json` files from the global, project, and legacy locations, so layering DefenseClaw across multiple files would cause duplicate hook firings.
